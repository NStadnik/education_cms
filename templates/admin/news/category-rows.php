    ...
    <tr>
        <td>
            <form id="categoryForm<?= e((string) $category['id']) ?>" method="post" action="<?= url('/admin/news/categories/save') ?>" data-replace-target="#newsCategoryRows" data-options-target="#newCategoryParent">
                ...
                <?php $categoryDepth = max(0, min(8, (int) ($category['depth'] ?? 0))); ?>
                <input class="news-category-title-input news-category-depth-<?= e((string) $categoryDepth) ?>" name="title" value="<?= e($category['title']) ?>" required aria-label="Назва категорії">
            </form>
        </td>
        <td>
